...
**A flow of payload execution process is as follows:**
  1) Hotplug device is inserted into target machine
  2) Terminal pops
  3) `Bash` memory logging is disabled and `Bash` history file is unset
  ...
  10) `exit` utilized to close terminal environment upon payload completion
